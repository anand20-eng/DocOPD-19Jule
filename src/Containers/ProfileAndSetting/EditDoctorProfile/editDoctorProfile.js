...
import { validationDoctorProfile } from './validation'
...
const EditDoctorProfile = ({theme}) => {
	const userDoctor = useSelector((state) => state.compAndDoc) || {};
	const [upDate, setUpdateId] = useState('')
	const [user, setUser] = useState({
		name: '',
		email: '',
		contact: userDoctor.contact,
		password: '',
		address: '',
		orgnisation_id: upDate.orgnisation_id,
		qualification: '',
		doctor_type: '',
		status: '1',
	})
	...
	useEffect(() => {
		getAllDoctor().then(response => {
			const getUpdateId = response.data.find(item => item.password == userDoctor.password) 
			 setUpdateId(getUpdateId)
			 setUser(getUpdateId)
		}).catch(error => {
			Alert.alert('data is not found');
	...
	const [roleValue, setRoleValue] = useState(userDoctor.role);
	...
	const editCompounder = () => {
		...
		validationDoctorProfile(user, upDate.id)
	}
	return (
